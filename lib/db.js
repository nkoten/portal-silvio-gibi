...
export async function getUsuarioByEmail(email) {
  try {
    ...
    // Se não houver nenhuma linha além do cabeçalho, retornamos null imediatamente
 ...
 */
export async function updateUsuario(email, dadosAtualizados) {
  const { googleSheets, spreadsheetId } = await getGoogleSheetsConnection();

  // 1. Procuramos em que linha o utilizador está
  const response = await googleSheets.spreadsheets.values.get({
    spreadsheetId,
    range: 'Usuarios!A:G',
  });
  ...
  // A linha exata na planilha (soma 1 porque arrays começam em 0 e as planilhas em 1)
  const numeroDaLinha = rowIndex + 1;

  // 2. Atualizamos as células específicas: B (Nome), F (Bio) e G (AvatarUrl)
  // Nota: Não atualizamos o email (C) nem o ID (A) por segurança.
  await googleSheets.spreadsheets.values.update({
    spreadsheetId,
    range: `Usuarios!B${numeroDaLinha}:B${numeroDaLinha}`, // Atualiza Nome
    valueInputOption: 'USER_ENTERED',
    requestBody: { values: [[dadosAtualizados.nome]] },
  });

  await googleSheets.spreadsheets.values.update({
    spreadsheetId,
    range: `Usuarios!F${numeroDaLinha}:G${numeroDaLinha}`, // Atualiza Bio e Avatar juntos
    valueInputOption: 'USER_ENTERED',
    requestBody: {
      values: [[dadosAtualizados.bio || '', dadosAtualizados.avatarUrl || '']],
    },
  });
...
export async function createComentario(comentarioData) {
  ...
  // Busca o próximo ID seguro na aba _meta
  const newId = await getNextId('Comentarios');
  const dataAtual = new Date().toISOString();

  // Insere a nova linha na aba Comentarios
  await googleSheets.spreadsheets.values.append({
    spreadsheetId,
...
export async function getComentariosPorPost(postId) {
  ...
  const response = await googleSheets.spreadsheets.values.get({
    spreadsheetId,
    range: 'Comentarios!A2:E', // Lemos todos os comentários ignorando o cabeçalho
  });

  const rows = response.data.values || [];

  // Filtramos os resultados: só queremos as linhas onde a coluna PostID (índice 1)
  // seja exatamente igual ao ID do post que estamos a visualizar.
  // Usamos == em vez de === porque o Google Sheets pode retornar o número como string
  const comentariosDoPost = rows.filter((row) => row[1] == postId);

  // Mapeamos para um objeto limpo e fácil de usar no frontend
